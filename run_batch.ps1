...
# 用法: powershell -ExecutionPolicy Bypass -File .\run_batch.ps1 [-ConfigDir configs]
...
try {
    Write-Host ("=== batch start {0} total={1} ===" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $files.Count)
    foreach ($f in $files) {
        Write-Host ""
        Write-Host "============================================================"
